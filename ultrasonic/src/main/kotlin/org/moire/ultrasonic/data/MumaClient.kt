...
class MumaClient(private val okHttpClient: OkHttpClient) {
    ...
    fun login(baseUrl: String, username: String, password: String): LoginResponse? {
        // Base URL could be something like http://192.168.1.27/lms
        // We want http://192.168.1.27/muma/api/auth/login
        val mumaUrl = if (baseUrl.contains("/lms")) {
            baseUrl.replace("/lms", "/muma") + "/api/auth/login"
        } else {
            baseUrl.removeSuffix("/") + "/muma/api/auth/login"
        }
        
        Timber.d("Muma login URL: $mumaUrl")
            ...
            okHttpClient.newCall(request).execute().use { response ->
                if (response.isSuccessful) {
                    ...
                    gson.fromJson(responseBody, LoginResponse::class.java).also {
                        ...
                        Timber.i("Muma login successful, API key saved")
                    }
                } else {
    ...
    fun getDynamicPlaylists(baseUrl: String): List<DynamicPlaylist> {
        ...
        val mumaUrl = if (baseUrl.contains("/lms")) {
            baseUrl.replace("/lms", "/muma") + "/api/users/$userId/dynamic-playlists"
        } else {
            baseUrl.removeSuffix("/") + "/muma/api/users/$userId/dynamic-playlists"
        }
